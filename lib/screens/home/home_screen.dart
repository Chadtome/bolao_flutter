...
class _HomeScreenState extends State<HomeScreen> with AutomaticKeepAliveClientMixin {
  ...
  Widget _buildWelcomeBanner() {
    ...
    return Container(
      key: const Key('welcome_banner'), // 👈 KEY ADICIONADA
      width: double.infinity,
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
        gradient: LinearGradient(
  ...
  Widget _buildRankingCard() {
    ...
    return Card(
      key: const Key('ranking_card'), // 👈 KEY ADICIONADA
      elevation: 2,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
      child: Column(
        children: [
          ...
          if (_expandedRanking) ...[
            ...top5.asMap().entries.map((entry) {
              ...
              return Container(
                key: ValueKey('top_${participante.id}'), // 👈 KEY ADICIONADA
                padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                child: Row(
                  children: [
                    SizedBox(
            ...
            if (ultimo != null)
              Container(
                key: const Key('lanterna_container'), // 👈 KEY ADICIONADA
                margin: const EdgeInsets.all(16),
                padding: const EdgeInsets.all(16),
                decoration: BoxDecoration(
                  color: Colors.red.shade50,
  ...
  Widget _buildProximaRodadaCard() {
    return Card(
      key: const Key('proxima_rodada_card'), // 👈 KEY ADICIONADA
      elevation: 2,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
      child: Column(
        children: [
            ...
            Padding(
              ...
              child: Column(
                children: _proximosJogos.asMap().entries.map((entry) {
                  ...
                  return Container(
                    key: ValueKey('jogo_$idx'), // 👈 KEY ADICIONADA
                    padding: const EdgeInsets.only(bottom: 12),
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
